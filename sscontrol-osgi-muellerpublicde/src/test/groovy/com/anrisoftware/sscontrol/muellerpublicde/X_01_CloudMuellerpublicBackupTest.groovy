 ...
 */
@Slf4j
class X_01_CloudMuellerpublicBackupTest extends Abstract_Runner_Debian_Test {
...
service "backup" with {
    ...
    destination arguments: "-c --delete", dir: "${dailyPath}/cloud-muellerpublic-de/mariadb"
    client key: k8s_vars.rsync.key, proxy: true, timeout: "1h"
}
service "backup" with {
    service namespace: "cloud-muellerpublic-de", name: "owncloud"
    destination arguments: "-c --delete", dir: "${dailyPath}/cloud-muellerpublic-de/owncloud"
    client key: k8s_vars.rsync.key, proxy: true, timeout: "1h"
    source << "/data"
    source << "/html"
}
